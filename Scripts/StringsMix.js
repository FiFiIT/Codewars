...
const mix = (s1, s2) => {
  var abc = crateABC();
  s1.match(/[a-z]/g).map((v) => updateABC(v, abc));
  s2.match(/[a-z]/g).map((v) => updateABC(v, abc, false));

  abc = abc.filter((l) => l.s1 > 1 || l.s2 > 1);
  abc = abc.reduce((acc, v, i, arr) => {
    if (v.s1 > 1 || v.s2 > 1) {
      var alpha = {};
      alpha.id = v.id;
      alpha.value = v.s1 > v.s2 ? v.s1 : v.s2;
      alpha.rank = 0;
  ...
  return abc
    .reduce((a, v) => {
      a.push([v.desc + v.id.repeat(v.value)]);
      return a;
    }, [])
    .join("/");
};
...
function crateABC() {
  return "abcdefghijklmnopqrstuvwxyz".split("").reduce((acc, v, i) => {
    var element = {};
    element.id = v;
    element.s1 = 0;
    element.s2 = 0;
...
function updateABC(letter, arr, s1 = true) {
  return arr.map((l) => {
    if (l.id == letter) {
      s1 ? (l.s1 += 1) : (l.s2 += 1);
    }
